    ...
    def __init__(self, django=True, *args, **kwargs):
            ...
            html.Div(
                [
                    ...
                    dcc.Checklist(id='mouse',
                                  options=[
                                      {'label': 'Only with mouse movements', 'value': 'movement'},
                                      {'label': 'Only with mouse clicks', 'value': 'click'},
        ...
        @self.app.callback(Output('geo_map', 'children'),
                           ...
                           [State('clear_bots', 'value'), State('remove_locations', 'value'),
                            State('date-picker-range', 'start_date'), State('date-picker-range', 'end_date'), State('mouse', 'value')])
        def update_graph(hidden_update, clear_bots, remove_locations, start_date, end_date, mouse_behavior):
            if end_date is None and start_date is not None:
                end_date = (datetime.datetime.strptime(start_date, '%Y-%m-%d') + datetime.timedelta(days=1)).strftime('%Y-%m-%d')
            pageview_stats = Pageview.objects.stats(start_date=start_date, end_date=end_date, filter_agents=clear_bots,
                                                    remove_locations=remove_locations, mouse_behavior=mouse_behavior)
            # a = 5
            fig = go.Figure()
